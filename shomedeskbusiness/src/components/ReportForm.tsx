...
import { Save, Calculator, FileText, Wallet, Landmark, ArrowRightLeft, History, Calendar, Banknote, Plus, Eye } from 'lucide-react';
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-bold">
                    {format(new Date(selectedReport.date), 'EEEE, MMM dd, yyyy')}
                  </p>
                </div>
                <button 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 bg-slate-900/30 p-4 rounded-2xl border border-slate-800/50">
                      <div className="flex justify-between text-xs">
                        <span className="text-slate-500 font-bold">Opening</span>
                        <span className="text-slate-300 font-black">{currency}{selectedReport.openingCash.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-500 font-bold">Sales (+)</span>
                        <span className="text-emerald-400 font-black">{currency}{selectedReport.cashSale.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-rose-500 font-bold">Expenses (-)</span>
                        <span className="text-rose-400 font-black">{currency}{selectedReport.cashExpense.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-blue-500 font-bold">Transfers (Net)</span>
                        <span className="text-blue-400 font-black">{currency}{(selectedReport.bankToCash - selectedReport.cashToBank).toLocaleString()}</span>
                      </div>
                      <div className="h-px bg-slate-800 my-2" />
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-200 font-black">Closing Cash</span>
                        <span className="text-emerald-400 font-black">{currency}{selectedReport.closingCash.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 bg-slate-900/30 p-4 rounded-2xl border border-slate-800/50">
                      <div className="flex justify-between text-xs">
                        <span className="text-slate-500 font-bold">Opening</span>
                        <span className="text-slate-300 font-black">{currency}{selectedReport.openingBank.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-500 font-bold">Sales (+)</span>
                        <span className="text-emerald-400 font-black">{currency}{selectedReport.bankSale.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-rose-500 font-bold">Expenses (-)</span>
                        <span className="text-rose-400 font-black">{currency}{selectedReport.bankExpense.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-blue-500 font-bold">Transfers (Net)</span>
                        <span className="text-blue-400 font-black">{currency}{(selectedReport.cashToBank - selectedReport.bankToCash).toLocaleString()}</span>
                      </div>
                      <div className="h-px bg-slate-800 my-2" />
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-200 font-black">Closing Bank</span>
                        <span className="text-blue-400 font-black">{currency}{selectedReport.closingBank.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-6 border-t border-slate-800 bg-slate-900/50">
                <button
                  onClick={() => setSelectedReport(null)}
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white py-4 rounded-2xl font-black transition-all"
                >
                  Close Report
                </button>
              </div>
